...
from classifier_tsrs import Classifier
...
def main(args: argparse.Namespace):
    ...
    backbone = utils.get_model(args.arch, args.pretrained, opt=args)
...
def train(train_iter: ForeverDataIterator, model: Classifier, bss_module, optimizer: SGD,
    ...
    noise_losses= AverageMeter('noiseloss', ':3.2f')
    cls_accs = AverageMeter('Cls Acc', ':3.1f')

    progress = ProgressMeter(
        args.iters_per_epoch,
        [batch_time, data_time, losses, noise_losses, cls_accs],
    ...
    for i in range(args.iters_per_epoch):
        ...
        # compute output
        y, f, noise_err = model(x)
        cls_loss = F.cross_entropy(y, label)
        bss_loss = bss_module(f)
        noise_loss = torch.mean(torch.stack(noise_err))
        cls_acc = accuracy(y, label)[0]
        loss = cls_loss + args.trade_off * bss_loss + args.lamba * noise_loss


        losses.update(loss.item(), x.size(0))
        noise_losses.update(noise_loss.item(), x.size(0))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--Nlayer', type=int, default='2', help="regulazation layer")
    parser.add_argument('--noise', type=float, default='0.2')
    parser.add_argument('--alpha', type=float, default=1,
                        help='TSRS alpha')
    parser.add_argument('--lamba', type=float, default=1,
                        help='init LSRN lamba')
